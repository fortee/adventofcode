...
def check_passphrase(passphrase):
    ...
    for word in words_array:
        ...
        ordered_array.append(order_word_letters(word))

        continue

    if len(ordered_array) != len(set(ordered_array)):
        # Check if we have any duplicate elements (anagrams) in the ordered list
        return False
...
def order_word_letters(word):
    """Order the letters of the word alphabetically"""
