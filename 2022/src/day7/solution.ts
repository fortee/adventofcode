 ...
 */
async function doit(input: string[]): Promise<void> {
...
function createDirectoryTree(input: string[]): void {
  ...
  for (const line of input) {
    ...
    switch (lineData[0]) {
      ...
      default:
        // This is a file
        addFilesSize(currentDirectory, +lineData[0]);
...
function createNewDirectory(directoryName: string, parentDirectory: dir): void {
  ...
  if (childWithNameExists !== undefined) {
    // We should not add the same child directory again
    return;
  }
  const directory: dir = {
    name: directoryName,
...
/**
 * Update the size of the directory, and it's parent (recursively)
 * TODO:  This is not the most efficient as for each file we traverse up the child-parent hierarchy
 *        We could achieve the same thing quicker by adding up the child directory sizes when we traverse the tree.
 * @param directory - The directory we want to update
 * @param size - The size of the file
 */
function addFilesSize(directory: dir, size: number): void {
  directory.size += size;
  if (directory.parent !== undefined) {
    addFilesSize(directory.parent, size);
  }
